...
class HeterogeneousHMM(_BaseHMM):
    """
        ...
        features.
        Parameters:
            n_states (int) - number of hidden states in the models
    ...
    def __init__(
        ...
        :param params:
        :param init_params:
        :param nr_no_train_de:
        :param state_no_train_de:
        :param init_type:
    ...
    def generate_sample_from_state(self, state):
        ...
        :return: X (array) - array of shape (n_gfeatures+n_dfeatures, ) containing a
    ...
    def _init(self, X):
        """
        Initialises models parameters prior to fitting.

        :param list X: list of observation sequences
        """
    ...
    def _reestimate_B(self, observations, gamma):
        """
        Re-estimation of the emission matrix (part of the 'M' step of Baum-Welch).
        Computes B_new = expected # times in state s_j with symbol v_k /expected
        # times in state s_j

        :param np.ndarray observations: array of shape (n_samples, n_d_features)
                containing the observation samples
        :param np.ndarray gamma: array of shape (n_samples, n_states)
        :return: B_new (dictionary) - the modified parts of the emission matrix
        """
        B_new = {
            "numer": [
                np.zeros((self.n_states, self.n_d_features[i]))
    ...
    def _calc_sigma(self, state, nan_index, sigma_flat):
        ...
        :param int state: the index of the hidden state to consider
        :param np.ndarray nan_index: contains the indices of NaN elements in the observation
        :param str sigma_flat: indicator of which calculation to use
        :return: res (array) - the computed covariance values
        """
